...
import { format, startOfMonth, parseISO, isSameDay, differenceInMinutes, differenceInDays } from 'date-fns';
...
function MAAppContent({ id }: { id: string }) {
  ...
  const handleClockAction = async (action: 'IN' | 'OUT', exitType?: 'PAUSE' | 'END') => {
    ...
    if (action === 'IN') {
      ...
      updateDocumentNonBlocking(doc(firestore, 'timeEntries', activeEntryId), {
